...
export default function Header({ hide = true }) {
  ...
  return (
    ...
    >
      <button
        onClick={() => router.push("/")}
        className="w-96 h-10 bg-cover bg-no-repeat bg-[url('/Logo.png')]"
      ></button>
      <button
        className="text-white text-lg font-semibold flex flex-row items-center gap-2 "
        onClick={clickTurul}
      >
        Төрөл
        <TurulIcon />
      ...
      <div
        className={
          turul
            ? "w-[42vh] h-[50vh] bg-[#353535] border-[1px] border-[#969084] rounded-sm relative right-40 top-56 cursor-pointer overflow-y-scroll"
            : "w-96 h-[50vh] bg-[#2B2B2B] border-[1px] border-white rounded-sm relative right-40 top-64 cursor-pointer overflow-y-scroll invisible"
        }
      >
